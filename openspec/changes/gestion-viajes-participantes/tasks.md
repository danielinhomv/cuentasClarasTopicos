...
- [ ] 1.1 Alcance: solo migración. Tipo: no funcional (persistencia). Crear el archivo de migración para la tabla `viajes` con columnas `id`, `user_id` (FK a `users` con onDelete cascade), `nombre` (string 150), `descripcion` (text nullable) y timestamps. No ejecutar `php artisan migrate`. Verificar: el archivo de migración existe en `database/migrations/` con la sintaxis correcta. Spec: vinculada a `viajes/spec.md` (Persistencia y protección de acceso a viajes).

- [ ] 1.2 Alcance: solo migración. Tipo: no funcional (persistencia e integridad). Crear el archivo de migración para la tabla `participantes` con columnas `id`, `viaje_id` (FK a `viajes` con onDelete cascade), `nombre` (string 100), timestamps y restricción única compuesta `unique(['viaje_id', 'nombre'])`. No ejecutar `php artisan migrate`. Verificar: el archivo de migración existe en `database/migrations/` con la restricción de unicidad. Spec: vinculada a `participantes/spec.md` (Nombre duplicado dentro del mismo viaje).

...
<template>
    <dialog class="z-50 m-0 min-h-full min-w-full overflow-y-auto bg-transparent backdrop:bg-transparent" ref="dialog">
        <div class="fixed inset-0 overflow-y-auto px-4 py-6 sm:px-0 z-50 flex items-center justify-center" scroll-region>
            ...
            >
                <div v-show="show" class="mb-6 bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden shadow-2xl shadow-cyan-950/40 transform transition-all sm:w-full sm:mx-auto text-zinc-100 relative z-10" :class="maxWidthClass">
                    <slot v-if="showSlot"/>
                </div>
            </transition>
        </div>
